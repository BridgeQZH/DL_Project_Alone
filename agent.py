...
class Agent(object):
    ...
    def save_model(self, epoch): 
        self.combined.save('nnnew_monet2photo_fliped_epoch_%d' %epoch)
        print("Saved model to disk")

    def save_data(self, epoch, alldata): 
        pickle.dump( alldata, open("nnnew_fliped_data%d.p" %epoch, "wb" ) )
        print("Saved data to disk")

    def sample_images(self, epoch, batch):
        save_path = os.path.join(self.save_dir, self.dataset_name)
        save_path = os.path.join(save_path, 'nnnew_fliped_256monet')
